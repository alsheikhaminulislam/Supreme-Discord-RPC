...
if (id !== undefined) {
	...
	if (options.largeimage !== '') {
		...
		activity.largeImageText = "Made with Al Sheikh Aminul Islam's Supreme Discord RPC (v2.1.1 CLI)!"
	}
	...
	function createWindow() {
		...
		if (os.platform() !== 'win32') {
			...
			setTimeout(() => {
				require('dns').resolve("https://alsheikhaminulislam.github.io/Supreme-Discord-RPC", function (err) {
					if (err) {
	...
	if (!gotTheLock) {
		...
		app.whenReady().then(() => {
			...
			contextMenu.append(new MenuItem({
				label: 'Show Supreme Discord RPC',
				click: () => { app.isquitting = true; win.show() }
			}))
			contextMenu.append(new MenuItem({
				label: 'Quit Supreme Discord RPC',
				click: () => { app.quit() }
			}))
			appIcon.setContextMenu(contextMenu)
			appIcon.setToolTip("Supreme Discord RPC")
		})
	}
