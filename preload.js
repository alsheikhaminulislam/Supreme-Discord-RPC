...
if (!fs.existsSync(dir)) {
	...
	const msg = {
		...
		defaultId: 0,
		title: 'Welcome',
		message: 'Thank you for choosing Supreme Discord RPC!',
...
document.addEventListener("DOMContentLoaded", () => {
	...
	document.getElementById("test").addEventListener("click", () => {
		...
		if (options.largeimage !== '') {
			...
			activity.largeImageText = "Made with Al Sheikh Aminul Islam's Supreme Discord RPC (v2.1)!"
		}
		if (options.smallimage !== '') {
			activity.smallImageKey = options.smallimage
			// Same applies with assets.large_text
			activity.smallImageText = 'https://alsheikhaminulislam.github.io/Supreme-Discord-RPC/'
		}
		...
		/*const myNotification = new Notification("Supreme Discord RPC", {
		...
		if (settings.launchedpresence == false) {
			...
			fs.writeFile(`${dir}${slash}settings.json`, JSON.stringify(settings, null, 2), 'utf8', (err) => {
				if (err) { throw err }
					...
					const msg = {
						type: 'question',
						buttons: ['Close', 'Star on GitHub', 'Join the Discord'],
						defaultId: 0,
						title: 'Welcome',
					...
					dialog.showMessageBox(null, msg).then(result => {
						if (result.response == 1) {
							shell.openExternal('https://github.com/alsheikhaminulislam/Supreme-Discord-RPC')
						}
						if (result.response == 2) {
							shell.openExternal('https://discord.com/invite/zzqdKZEdwF')
						}
					})
				}
			})
				...
				fs.stat(fullpath, function (err, stat) {
					if (err == null) { //get if it exists
						...
						fs.unlink(fullpath, (err) => {
							...
							const myNotification = new Notification("Supreme Discord RPC", { //throw a notification
	...
	registerLinkToOpenInBrowser("github-button", "https://github.com/alsheikhaminulislam")
	registerLinkToOpenInBrowser("discord-button", "https://discord.com/invite/zzqdKZEdwF")
	registerLinkToOpenInBrowser("web-button", "https://alsheikhaminulislam.github.io/Supreme-Discord-RPC")
	registerLinkToOpenInBrowser("youtube-button", "https://www.youtube.com/channel/UCfctLPzPJZBj25yMS_lcGaA")
	...
	registerLinkToOpenInBrowser("cliutility", "https://github.com/alsheikhaminulislam/Supreme-Discord-RPC-CLI")
